...
<Callout type="error">
  ...
  It is strongly recommended to have a separate go2rtc instance for production (e.g. via HACS)
 </Callout>
...
- A websocket URL used to override the default HA Go2RTC server url
...
- Expects a `WebRTCConnection` from `useWebRTC`, which can be found in `@/scripts/video-rtc.tsx`.
